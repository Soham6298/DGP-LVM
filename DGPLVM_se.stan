// DGP-LVM with squared exponential (SE) covariance function 
// GP Paramters:
// GP length scale: rho
...
functions {
  ...
  matrix deriv_se(vector x, int[] derivative, real alpha_obs, real alpha_grad, real rho, real delta) {
    int N = rows(x);
    matrix[N, N] K;
